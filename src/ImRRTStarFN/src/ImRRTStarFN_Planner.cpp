...
#include "ImRRTStarFN/ImRRTStarFN_Planner.h"

// register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(rrt_starFN_Global_planner::ImRRTStarFN_planner, nav_core::BaseGlobalPlanner)


namespace rrt_starFN_Global_planner {

ImRRTStarFN_planner::ImRRTStarFN_planner() : costmap_(nullptr), initialized_(false) {
    ROS_INFO("--- Hello, this is ImRRTStarFN_planner default constructor ---");
}

ImRRTStarFN_planner::ImRRTStarFN_planner(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
                                    : costmap_(nullptr), initialized_(false) {
    // initialize the planner
    initialize(name, costmap_ros);
}

ImRRTStarFN_planner::ImRRTStarFN_planner(std::string name, costmap_2d::Costmap2D* costmap, std::string global_frame) 
                                    : costmap_(nullptr), initialized_(false) {
    // initialize the planner
...
void ImRRTStarFN_planner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros){
    initialize(name, costmap_ros->getCostmap(), costmap_ros->getGlobalFrameID());
}

void ImRRTStarFN_planner::initialize(std::string name, costmap_2d::Costmap2D* costmap, std::string global_frame) {
    if(!initialized_) {
        costmap_ = costmap;
        ...
        ROS_INFO("ImRRTStarFN Global Planner initialized successfully.");
        initialized_ = true;
    }
    else {
        ROS_WARN("This ImRRTStarFN planner has already been initialized... doing nothing.");
    }
    
}

void ImRRTStarFN_planner::computeFinalPlan(std::vector<geometry_msgs::PoseStamped>& plan, std::list<std::pair<double, double>>& path) {
    plan.clear();
...
bool ImRRTStarFN_planner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan){
    
    plan.clear();

    ROS_INFO(" --- ImRRTStarFN Global planner ---- ");
    ...
    if (planner_->pathPlanning(path)) {
        ROS_INFO("ImRRTStarFN Global Planner: Path found!!!!");
        // convert path to plan
        computeFinalPlan(plan, path);
        // // 输出到文件
        // std::ofstream file("/home/lizy/demo_mynav/path.csv", std::ios::out);
        ...
        ROS_WARN("ImRRTStarFN Global Planner fail to find path, choose other goal.");
        return false;
    }
}

void ImRRTStarFN_planner::publishPlan(std::vector<geometry_msgs::PoseStamped>& plan) {
    if (!initialized_) {
...
ImRRTStarFN_planner::~ImRRTStarFN_planner() { }
