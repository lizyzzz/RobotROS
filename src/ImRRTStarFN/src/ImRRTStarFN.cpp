#include "ImRRTStarFN/ImRRTStarFN.h"
...
ImRRTStarFN::ImRRTStarFN(const std::pair<double, double>& start_point, 
            ...
            { 
                ...
                deltaCount_ = static_cast<int>(std::floor(max_num_nodes_ / dynamic_count_));
                // ROS_ERROR("deltaR : %.2f, deltaCount: %d", deltaR_, deltaCount_);
                // deltaR_ = 0;
                // deltaCount_ = static_cast<int>(std::floor(max_num_nodes_ / dynamic_count_));
                // set range
    ...
    while (node_count_ < max_num_nodes_) {
        ...
        if (goal_reached_ && node_count_ >= min_num_nodes_) {
            if (goal_reached_cnt == false) {
                ...
                goal_node_.leaf = 1;
                nodes_.insert(std::make_pair(node_count_, goal_node_)); // insert
...
int ImRRTStarFN::getNearestNodeId(const std::pair<double, double> &point) {
    ...
    Node node_nearest = nodes_.begin()->second;
    // find
...
void ImRRTStarFN::creatNewNode(double x, double y, int node_nearest_id) {
    ...
    nodes_.insert(std::make_pair(node_count_, new_node));
...
void ImRRTStarFN::force_remove() {
    ...
    if (max_cost_id >= 0) {
        ...
        // delete it from nodes_
        nodes_.erase(max_cost_id);
        return;
    }
...
void ImRRTStarFN::computeFinalPath(std::list<std::pair<double, double>>& path) {
    ...
    goal_node_ = nodes_.at(goal_node_.node_id); // update goal_node
